...
const ProjectCard = ({project}) => {
    const { title, description, image, tech, metrics, demo, github } = project;
  ...
  return (
    <div className="group relative bg-white/5 border-white/10 rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-300">
      ...
      <div className="p-6 space-y-4">
        ...
        {metrics && (
          <div className="flex items-center gap-2 gt-3 border-t border-white/10">
            <TrendingUp className="w-4 h-4 text-cyan-500" />
            <p className="text-sm font-medium text-cyan-500"> {metrics} </p>
          </div>
        )}
      </div>
    </div>
