...
const RadialGradientBackground = ({ variant= 'hero', gradients= [] }) => {
    ...
    const varients = {
        hero: [
          ...
          },
          {
            ...
            size: 'w-[1400px] h-[1400px]',
            colors: [
              { color: 'rgba(105, 180, 255, 0.25)', stop: '100%' },
              { color: 'rgba(105, 180, 255, 0.45)', stop: '100%' },
              { color: 'rgba(105, 180, 255, 0.5)', stop: '100%' },
              { color: 'rgba(105, 180, 255, 0.45)', stop: '100%' },
              { color: 'rgba(105, 180, 255, 0.25)', stop: '100%' }
            ],
            blur: '0px',
          ...
          },
          {
            ...
            size: 'w-[1400px] h-[1400px]',
            colors: [
              { color: 'rgba(255, 105, 180, 0.25)', stop: '100%' },
              { color: 'rgba(255, 105, 180, 0.45)', stop: '100%' },
              { color: 'rgba(255, 105, 180, 0.5)', stop: '100%' },
              { color: 'rgba(255, 105, 180, 0.45)', stop: '100%' },
              { color: 'rgba(255, 105, 180, 0.25)', stop: '100%' }
            ],
            blur: '0px',
  ...
  return (
    <div clasName="absolute inset-0 overflow-hidden pointer-events-none">
       {activeGradients.map((gradient, index) => (
        <div
          key={index}
          className={`absolute ${gradient.position} ${gradient.size} rounded-full filter blur-[${gradient.blur}] opacity-${gradient.opacity}`}
